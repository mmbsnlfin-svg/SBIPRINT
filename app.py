...
def unique_filename(base_name: str, used_names: set[str], extension: str) -> str:
    """Avoid overwriting files when two split values clean to the same name."""
...
def copy_header_merged_cells(src_ws, dst_ws) -> None:
    """Copy merged ranges that are fully contained within header rows 1 and 2."""
    for merged_range in src_ws.merged_cells.ranges:
        if merged_range.min_row >= 1 and merged_range.max_row <= 2:
            dst_ws.merge_cells(str(merged_range))


def apply_fixed_column_widths(ws, max_col: int) -> None:
    for col_num, width in enumerate(FIXED_COLUMN_WIDTHS, start=1):
        if col_num <= max_col:
...
def apply_pdf_page_settings(workbook_path: Path) -> None:
    """Apply print settings before LibreOffice converts the file to PDF."""
...
def split_excel_file(
    ...
    try:
        ...
        for split_value in unique_values:
            ...
            old_font = split_header.font
            split_header.font = Font(
                name=old_font.name,
                size=old_font.size,
                bold=True,
            ...
            split_header.border = THIN_BORDER
            ...
            for col_idx in range(1, max_col + 1):
                total_cell = new_ws.cell(row=total_row, column=col_idx)
                old_total_font = total_cell.font
                total_cell.font = Font(
                    name=old_total_font.name,
                    size=old_total_font.size,
                    bold=True,
                    italic=old_total_font.italic,
                    color=old_total_font.color,
                )
                total_cell.border = THIN_BORDER

            if apply_fixed_widths:
                apply_fixed_column_widths(new_ws, max_col)
...
def main() -> None:
    ...
    st.caption("Upload one Excel file, split it by a selected column, and download Excel and/or PDF files.")
    ...
    apply_widths = st.checkbox("Apply the fixed column widths from the desktop program", value=True)
        ...
        if uploaded_file is None:
            ...
            try:
                ...
                with st.status("Processing workbook...", expanded=True) as status:
                    st.write("Reading the uploaded workbook and identifying split values...")
                    excel_files, warnings = split_excel_file(
                        ...
                        apply_fixed_widths=apply_widths,
                    )
                    ...
                    if generate_pdf_now:
                        st.write("Converting the split Excel files to PDF...")
